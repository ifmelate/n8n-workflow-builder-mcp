...
const workflowValidator_1 = require("./validation/workflowValidator");
const nodeTypesLoader_1 = require("./validation/nodeTypesLoader");
...
server.tool("create_workflow", "Create a new n8n workflow", createWorkflowParamsSchema.shape, async (params, _extra) => {
...
server.tool("list_workflows", "List workflows in the workspace", listWorkflowsParamsSchema.shape, async (params, _extra) => {
...
server.tool("get_workflow_details", "Get workflow details by name or path", getWorkflowDetailsParamsSchema.shape, async (params, _extra) => {
    const workflowName = params.workflow_name;
    console.error("[DEBUG] get_workflow_details called with name:", workflowName);
    try {
        let filePath = (0, workspace_1.resolveWorkflowPath)(workflowName, params.workflow_path);
        // Auto-detect workspace when only workflow_name is provided and default path doesn't exist
        try {
            if (!params.workflow_path) {
                await promises_1.default.access(filePath).catch(async () => {
                    const detected = await (0, workspace_1.tryDetectWorkspaceForName)(workflowName);
                    if (detected)
                        filePath = detected;
                });
            }
...
const addNodeParamsSchema = zod_1.z.object({
    ...
    workflow_path: zod_1.z.string().optional().describe("Optional direct path to the workflow file (absolute or relative to current working directory). If not provided, uses standard workflow_data directory approach."),
    connect_from: zod_1.z.array(zod_1.z.object({
        source_node_id: zod_1.z.string().describe("Existing node ID to connect FROM"),
        source_node_output_name: zod_1.z.string().describe("Output handle on the source node (e.g., 'main' or 'ai_tool')"),
        target_node_input_name: zod_1.z.string().default('main').describe("Input handle on the new node"),
        target_node_input_index: zod_1.z.number().optional().default(0).describe("Input index on the new node (default: 0)")
    })).optional().describe("Optional: create connections from existing nodes to this new node"),
    connect_to: zod_1.z.array(zod_1.z.object({
        target_node_id: zod_1.z.string().describe("Existing node ID to connect TO"),
        source_node_output_name: zod_1.z.string().describe("Output handle on the new node (e.g., 'main' or 'ai_languageModel')"),
        target_node_input_name: zod_1.z.string().default('main').describe("Input handle on the target node"),
        target_node_input_index: zod_1.z.number().optional().default(0).describe("Input index on the target node (default: 0)")
    })).optional().describe("Optional: create connections from this new node to existing nodes")
});
server.tool("add_node", "Add a node to an n8n workflow", addNodeParamsSchema.shape, async (params, _extra) => {
    ...
    try {
        ...
        if (!(0, cache_1.isNodeTypeSupported)(finalNodeType, finalTypeVersion)) {
            ...
            const supported = (0, versioning_1.getN8nVersionInfo)()?.supportedNodes.get(finalNodeType);
            if (supported && supported.size > 0) {
                const sorted = Array.from(supported).map(v => Number(v)).filter(v => !isNaN(v)).sort((a, b) => b - a);
                const fallback = sorted[0];
                if (fallback !== undefined) {
                    console.warn(`[WARN] Requested ${finalNodeType}@${finalTypeVersion} not supported on ${(0, versioning_1.getCurrentN8nVersion)()}; falling back to ${fallback}`);
        ...
        if (isLangChainLLM) {
            ...
            if (params.parameters?.options?.credentials?.providerType === 'openAi') {
                ...
                if (nodeParameters.options?.credentials) {
                    ...
                    const credId = (0, id_1.generateN8nId)();
                    nodeParameters.credentials = {
                        "openAiApi": {
                            "id": credId,
                            "name": "OpenAi account"
        ...
        try {
            ...
            const nodeTypeDef = nodeTypes.getByNameAndVersion(finalNodeType, resolvedVersion);
            const credsCfg = nodeTypeDef?.description?.credentialsConfig;
            if (Array.isArray(credsCfg)) {
                for (const cfg of credsCfg) {
                    if (cfg?.required) {
                        const key = String(cfg.name);
                        if (!nodeCredentials[key]) {
                            nodeCredentials[key] = { id: (0, id_1.generateN8nId)(), name: `${key}-placeholder` };
                        }
                    }
                }
            }
            ...
            console.warn('[WARN] Could not compute credential placeholders for node:', e?.message || e);
        }
        const newNode = {
        ...
        try {
            if (params.connect_from && params.connect_from.length > 0) {
                for (const instr of params.connect_from) {
                    ...
                    if (!workflow.connections[sourceNameKey])
                        workflow.connections[sourceNameKey] = {};
                    if (!workflow.connections[sourceNameKey][outName])
                        workflow.connections[sourceNameKey][outName] = [];
                    workflow.connections[sourceNameKey][outName].push([{ node: targetNode.name, type: inName, index: inIndex }]);
...
const editNodeParamsSchema = zod_1.z.object({
    ...
    connect_from: zod_1.z.array(zod_1.z.object({
        source_node_id: zod_1.z.string().describe("Existing node ID to connect FROM"),
        source_node_output_name: zod_1.z.string().describe("Output handle on the source node (e.g., 'main' or 'ai_tool')"),
        target_node_input_name: zod_1.z.string().default('main').describe("Input handle on this node"),
        target_node_input_index: zod_1.z.number().optional().default(0).describe("Input index on this node (default: 0)")
    })).optional().describe("Optional: create connections from existing nodes to this node after edit"),
    connect_to: zod_1.z.array(zod_1.z.object({
        target_node_id: zod_1.z.string().describe("Existing node ID to connect TO"),
        source_node_output_name: zod_1.z.string().describe("Output handle on this node (e.g., 'main' or 'ai_languageModel')"),
        target_node_input_name: zod_1.z.string().default('main').describe("Input handle on the target node"),
        target_node_input_index: zod_1.z.number().optional().default(0).describe("Input index on the target node (default: 0)")
    })).optional().describe("Optional: create connections from this node to existing nodes after edit")
});
server.tool("edit_node", "Edit an existing node in a workflow", editNodeParamsSchema.shape, async (params, _extra) => {
    ...
    try {
        ...
        if (params.node_type) {
            ...
            if (!(0, cache_1.isNodeTypeSupported)(newType, newTypeVersion)) {
                const supported = (0, versioning_1.getN8nVersionInfo)()?.supportedNodes.get(newType);
                if (supported && supported.size > 0) {
                    const sorted = Array.from(supported).map(v => Number(v)).filter(v => !isNaN(v)).sort((a, b) => b - a);
                    if (sorted.length > 0)
                        newTypeVersion = sorted[0];
                }
            }
        }
        ...
        try {
            const nodeTypes = await (0, nodeTypesLoader_1.loadNodeTypesForCurrentVersion)(path_1.default.resolve(__dirname, '../workflow_nodes'), (0, versioning_1.getCurrentN8nVersion)());
            const typeDef = nodeTypes.getByNameAndVersion(nodeToEdit.type, nodeToEdit.typeVersion);
            const credsCfg = typeDef?.description?.credentialsConfig;
            if (Array.isArray(credsCfg) && credsCfg.length > 0) {
                nodeToEdit.credentials = nodeToEdit.credentials || {};
                for (const cfg of credsCfg) {
                    if (cfg?.required) {
                        const key = String(cfg.name);
                        if (!nodeToEdit.credentials[key]) {
                            nodeToEdit.credentials[key] = { id: (0, id_1.generateN8nId)(), name: `${key}-placeholder` };
                        }
                    }
                }
            }
...
server.tool("delete_node", "Delete a node from a workflow", deleteNodeParamsSchema.shape, async (params, _extra) => {
...
server.tool("add_connection", "Create a connection between two nodes", addConnectionParamsSchema.shape, async (params, _extra) => {
    ...
    try {
        let filePath = (0, workspace_1.resolveWorkflowPath)(workflow_name, params.workflow_path);
        try {
            if (!params.workflow_path) {
                await promises_1.default.access(filePath).catch(async () => {
                    const detected = await (0, workspace_1.tryDetectWorkspaceForName)(workflow_name);
                    if (detected)
                        filePath = detected;
                });
            }
        ...
        try {
            ...
            if (!report.ok || (report.warnings && report.warnings.length > 0)) {
                if (!report.ok)
                    console.warn('[WARN] Workflow validation failed after add_connection', report.errors);
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify({
...
const addAIConnectionsParamsSchema = zod_1.z.object({
    ...
    embeddings_node_id: zod_1.z.string().optional().describe("The ID of the embeddings node (optional)"),
    vector_store_node_id: zod_1.z.string().optional().describe("The ID of the vector store node (optional)"),
    vector_insert_node_id: zod_1.z.string().optional().describe("The ID of the vector store insert node (optional)"),
    vector_tool_node_id: zod_1.z.string().optional().describe("The ID of the Vector Store Question Answer Tool node (optional)"),
    workflow_path: zod_1.z.string().optional().describe("Optional direct path to the workflow file (absolute or relative to current working directory). If not provided, uses standard workflow_data directory approach.")
});
server.tool("add_ai_connections", "Wire AI model, tools, and memory to an agent", addAIConnectionsParamsSchema.shape, async (params, _extra) => {
    ...
    try {
        let filePath = (0, workspace_1.resolveWorkflowPath)(workflow_name, workflow_path);
        ...
        try {
            ...
            const nodeTypes = await (0, nodeTypesLoader_1.loadNodeTypesForCurrentVersion)(workflowNodesRootDir, (0, versioning_1.getCurrentN8nVersion)() || undefined);
            const agentType = nodeTypes.getByNameAndVersion(agentNode.type, agentNode.typeVersion);
            const role = agentType?.description?.wiring?.role;
            ...
            console.warn('[WARN] Could not verify agent node type against node definitions:', e?.message || e);
        }
        let modelNode = null;
        if (model_node_id) {
        ...
        if (toolNodes.length > 0) {
            for (const toolNode of toolNodes) {
                ...
                const exists = workflow.connections[toolNodeName]["ai_tool"].some((group) => Array.isArray(group) && group.some((d) => d && d.node === agentNode.name && d.type === "ai_tool"));
                if (!exists) {
        ...
        if (vectorStoreNode && vectorInsertNode) {
            ...
            if (!workflow.connections[fromName])
                workflow.connections[fromName] = {};
            if (!workflow.connections[fromName]["ai_document"])
                workflow.connections[fromName]["ai_document"] = [];
            const exists = workflow.connections[fromName]["ai_document"].some((group) => Array.isArray(group) && group.some((d) => d && d.node === toName && d.type === "ai_document"));
            if (!exists) {
                workflow.connections[fromName]["ai_document"].push([{ node: toName, type: "ai_document", index: 0 }]);
        ...
        if (vectorStoreNode) {
            ...
            if (toNode) {
                ...
                if (!workflow.connections[fromName])
                    workflow.connections[fromName] = {};
                if (!workflow.connections[fromName]["ai_vectorStore"])
                    workflow.connections[fromName]["ai_vectorStore"] = [];
                const exists = workflow.connections[fromName]["ai_vectorStore"].some((group) => Array.isArray(group) && group.some((d) => d && d.node === toName && d.type === "ai_vectorStore"));
                if (!exists) {
                    workflow.connections[fromName]["ai_vectorStore"].push([{ node: toName, type: "ai_vectorStore", index: 0 }]);
        ...
        if (modelNode) {
            ...
            if (toNode) {
                ...
                if (!workflow.connections[fromName])
                    workflow.connections[fromName] = {};
                if (!workflow.connections[fromName]["ai_languageModel"])
                    workflow.connections[fromName]["ai_languageModel"] = [];
                const exists = workflow.connections[fromName]["ai_languageModel"].some((group) => Array.isArray(group) && group.some((d) => d && d.node === toName && d.type === "ai_languageModel"));
                if (!exists) {
                    workflow.connections[fromName]["ai_languageModel"].push([{ node: toName, type: "ai_languageModel", index: 0 }]);
...
const composeAIWorkflowParamsSchema = zod_1.z.object({
    workflow_name: zod_1.z.string().describe("Name of the workflow to compose/update"),
    n8n_version: zod_1.z.string().optional().describe("Target n8n version to use for node catalogs and compatibility"),
    plan: zod_1.z.object({
        agent: zod_1.z.object({ node_type: zod_1.z.string().default("@n8n/n8n-nodes-langchain.agent"), node_name: zod_1.z.string().default("AI Agent") }).optional(),
        model: zod_1.z.object({ node_type: zod_1.z.string().default("@n8n/n8n-nodes-langchain.lmChatOpenAi"), node_name: zod_1.z.string().default("OpenAI Chat Model"), parameters: zod_1.z.record(zod_1.z.string(), zod_1.z.any()).optional() }).optional(),
        memory: zod_1.z.object({ node_type: zod_1.z.string().default("@n8n/n8n-nodes-langchain.memoryBufferWindow"), node_name: zod_1.z.string().default("Conversation Memory"), parameters: zod_1.z.record(zod_1.z.string(), zod_1.z.any()).optional() }).optional(),
        embeddings: zod_1.z.object({ node_type: zod_1.z.string().default("@n8n/n8n-nodes-langchain.embeddingsOpenAi"), node_name: zod_1.z.string().default("OpenAI Embeddings"), parameters: zod_1.z.record(zod_1.z.string(), zod_1.z.any()).optional() }).optional(),
        vector_store: zod_1.z.object({ node_type: zod_1.z.string().default("@n8n/n8n-nodes-langchain.vectorStoreInMemory"), node_name: zod_1.z.string().default("In-Memory Vector Store"), parameters: zod_1.z.record(zod_1.z.string(), zod_1.z.any()).optional() }).optional(),
        vector_insert: zod_1.z.object({ node_type: zod_1.z.string().default("@n8n/n8n-nodes-langchain.vectorStoreInMemoryInsert"), node_name: zod_1.z.string().default("In-Memory Vector Insert"), parameters: zod_1.z.record(zod_1.z.string(), zod_1.z.any()).optional() }).optional(),
        vector_tool: zod_1.z.object({ node_type: zod_1.z.string().default("@n8n/n8n-nodes-langchain.toolVectorStore"), node_name: zod_1.z.string().default("Vector QA Tool"), parameters: zod_1.z.record(zod_1.z.string(), zod_1.z.any()).optional() }).optional(),
        tools: zod_1.z.array(zod_1.z.object({ node_type: zod_1.z.string(), node_name: zod_1.z.string().optional(), parameters: zod_1.z.record(zod_1.z.string(), zod_1.z.any()).optional() })).optional(),
        trigger: zod_1.z.object({ node_type: zod_1.z.string().default("@n8n/n8n-nodes-langchain.chatTrigger"), node_name: zod_1.z.string().default("Start Chat Trigger"), parameters: zod_1.z.record(zod_1.z.string(), zod_1.z.any()).optional() }).optional()
    }).describe("High level plan of nodes to add and wire")
});
server.tool("compose_ai_workflow", "Compose a complex AI workflow (agent + model + memory + embeddings + vector + tools + trigger) in one call, including wiring and basic validation.", composeAIWorkflowParamsSchema.shape, async (params, _extra) => {
    ...
    try {
        ...
        const wiringResult = await (async () => {
            ...
            const resp = await (async () => {
                const parsed = await addAIConnectionsParamsSchema.parseAsync(toolParams);
            ...
            const connect = async (from, fromOutput, to, toInput) => {
                if (!from || !to)
                    return;
                let wfRaw = JSON.parse(await promises_1.default.readFile(filePath, 'utf8'));
                if (!wfRaw.connections)
                    wfRaw.connections = {};
                const fromName = from.name;
                if (!wfRaw.connections[fromName])
                    wfRaw.connections[fromName] = {};
                if (!wfRaw.connections[fromName][fromOutput])
                    wfRaw.connections[fromName][fromOutput] = [];
                const exists = wfRaw.connections[fromName][fromOutput].some((group) => Array.isArray(group) && group.some((d) => d && d.node === to.name && d.type === toInput));
                if (!exists)
                    wfRaw.connections[fromName][fromOutput].push([{ node: to.name, type: toInput, index: 0 }]);
                await promises_1.default.writeFile(filePath, JSON.stringify(wfRaw, null, 2));
            };
        ...
        // Validate
        const nodeTypes = await (0, nodeTypesLoader_1.loadNodeTypesForCurrentVersion)(path_1.default.resolve(__dirname, '../workflow_nodes'), (0, versioning_1.getCurrentN8nVersion)());
        const finalRaw = await promises_1.default.readFile(filePath, 'utf8');
        const finalWf = JSON.parse(finalRaw);
        const report = (0, workflowValidator_1.validateAndNormalizeWorkflow)(finalWf, nodeTypes);
        return {
...
const listAvailableNodesParamsSchema = zod_1.z.object({
    search_term: zod_1.z.string().optional().describe("Filter by name, type, or description. For LangChain nodes, try 'langchain', roles like 'agent', 'lmChat'/'llm', 'tool', 'memory', or provider names such as 'qdrant', 'weaviate', 'milvus', 'openai', 'anthropic'."),
...
server.tool("list_available_nodes", "List available node types. Supports tag-style synonym search and multi-token queries. By default, multiple terms use OR logic (e.g., 'webhook trigger' matches either). Set token_logic='and' to require all tokens. Disable synonyms with tags=false. Tips: search 'langchain', roles like 'agent', 'lmChat/llm', 'tool', 'memory', or provider names like 'qdrant', 'weaviate', 'milvus', 'openai', 'anthropic'.", listAvailableNodesParamsSchema.shape, async (params, _extra) => {
    ...
    try {
        ...
        if (params.search_term && params.search_term.trim() !== "") {
            ...
            const expandedTokensSet = new Set(baseTokens);
            if (useTags) {
                for (const t of baseTokens) {
                    if (synonymMap[t]) {
                        for (const syn of synonymMap[t])
                            expandedTokensSet.add(syn);
                    }
                }
            }
            ...
            availableNodes = allParsedNodes.filter(node => {
                const parts = [];
                if (node.displayName)
                    parts.push(String(node.displayName));
                if (node.nodeType)
                    parts.push(String(node.nodeType));
                if (node.description)
                    parts.push(String(node.description));
                if (node.simpleName)
                    parts.push(String(node.simpleName));
                if (node.categories && Array.isArray(node.categories))
                    parts.push(...node.categories.map((c) => String(c)));
                if (node.properties && Array.isArray(node.properties)) {
                    for (const prop of node.properties) {
                        if (prop?.name)
                            parts.push(String(prop.name));
                        if (prop?.displayName)
                            parts.push(String(prop.displayName));
                        if (prop?.options && Array.isArray(prop.options)) {
                            for (const opt of prop.options) {
                                if (opt?.name)
                                    parts.push(String(opt.name));
                                if (opt?.value)
        ...
        const formattedNodes = versionFilteredNodes.map(node => {
            const targetVersionInfo = (0, versioning_1.getSupportedN8nVersions)().get(targetVersion || (0, versioning_1.getCurrentN8nVersion)() || "1.108.0");
            const supportedVersions = targetVersionInfo?.supportedNodes.get(node.nodeType);
            ...
            return {
                ...
                propertiesPreview: (() => {
                    try {
                        ...
                        const MAX_PROPS = 5;
                        return props.slice(0, MAX_PROPS).map((p) => {
                            ...
                            if (typeof p?.default !== 'undefined')
                                preview.default = p.default;
                            if (p?.required === true)
                                preview.required = true;
                            if (optionValues && optionValues.length)
                                preview.optionValues = optionValues;
                            return preview;
                        });
                    }
                    catch {
...
server.tool("get_n8n_version_info", "Get N8N version info and capabilities", {}, async (_params, _extra) => {
...
const validateWorkflowParamsSchema = zod_1.z.object({
    workflow_name: zod_1.z.string().describe("The Name of the workflow to validate"),
    workflow_path: zod_1.z.string().optional().describe("Optional direct path to the workflow file")
});
server.tool("validate_workflow", "Validate a workflow file against known node schemas", validateWorkflowParamsSchema.shape, async (params, _extra) => {
    console.error("[DEBUG] validate_workflow called with:", params);
    try {
        let filePath = (0, workspace_1.resolveWorkflowPath)(params.workflow_name, params.workflow_path);
        try {
            if (!params.workflow_path) {
                await promises_1.default.access(filePath).catch(async () => {
                    const detected = await (0, workspace_1.tryDetectWorkspaceForName)(params.workflow_name);
                    if (detected)
                        filePath = detected;
                });
            }
        ...
        try {
            ...
            try {
                for (const [src, byType] of Object.entries(connectionsBySource)) {
                    ...
                    if (hasBoolean || numericKeys.length > 0)
                        legacyBranchNodes.push({ name: src, hasBoolean, numericKeys });
                }
            }
            ...
            if (startNode) {
                ...
                const getMainLikeGroups = (byType) => {
                    ...
                    if (Array.isArray(groupsMain) && groupsMain.length > 0)
                        return groupsMain;
                    const tfTrue = Array.isArray((byType || {}).true) ? (byType || {}).true : [];
                    const tfFalse = Array.isArray((byType || {}).false) ? (byType || {}).false : [];
                    if (tfTrue.length > 0 || tfFalse.length > 0)
                        return [...tfTrue, ...tfFalse];
                ...
                for (const [src, byType] of Object.entries(connectionsBySource)) {
                    ...
                    for (const group of groups || []) {
                        for (const conn of group || []) {
                            if (!conn)
                                continue;
                            if (!mainNeighbors[src])
                                mainNeighbors[src] = new Set();
                ...
                const addUndirected = (a, b) => {
                    if (!allNeighbors[a])
                        allNeighbors[a] = new Set();
                    if (!allNeighbors[b])
                        allNeighbors[b] = new Set();
                    ...
                    for (const entry of legacyBranchNodes) {
                        ...
                        if (entry.hasBoolean && entry.numericKeys.length > 0) {
                            allErrors.push({
                                ...
                                nodeName: entry.name,
                                details: { nodeId: node.id, type: node.type, keys: shapeKeys }
                            });
                        }
                            ...
                            allErrors.push({
                                ...
                                nodeName: entry.name,
                                details: { nodeId: node.id, type: node.type, keys: entry.numericKeys }
                            });
                        }
                    }
                }
            ...
            console.warn('[WARN] Connectivity validation errored in validate_workflow:', e?.message || e);
        }
        ...
        try {
            ...
            const nodesByName = normalized.nodes || {};
            const connectionsBySource = normalized.connectionsBySource || {};
            const getTypeDesc = (node) => node ? nodeTypes.getByNameAndVersion(node.type, node.typeVersion) : undefined;
            const producersByType = {};
            for (const node of Object.values(nodesByName)) {
                const desc = getTypeDesc(node);
                const produces = ((desc?.description?.wiring?.produces) || []);
            ...
            const addAgentWireSuggestion = (agent, model, tools, memory) => {
                suggestedActions.push({
            ...
            for (const issue of [...report.warnings]) {
                ...
                const desc = getTypeDesc(node);
                const role = desc?.description?.wiring?.role;
                if (issue.code === 'unconnected_node' && node) {
                    if (role === 'vectorStore') {
                        ...
                        const vectorStores = Object.values(nodesByName).filter(n => (getTypeDesc(n)?.description?.wiring?.role) === 'vectorStore');
                ...
                if (issue.code === 'missing_required_input' && node && issue.details?.input) {
                    const req = String(issue.details.input);
                    ...
                    if (candidates.length > 0) {
                        ...
                        if ((getTypeDesc(node)?.description?.wiring?.role) === 'agent' && req.toLowerCase() === 'ailanguagemodel') {
            ...
            console.warn('[WARN] Failed to compute remediation suggestions in validate_workflow:', e?.message || e);
        }
